...
# `Inches`, `en_GB` with `en_US`, and `true` with `sfalse`.
...
defaults write ch.sudo.cyberduck rendezvous.enable falsedefaults write ch.sudo.cyberduck rendezvous.enable false
